...
double NmeaParser::nmeaToDecimal(std::string_view val, std::string_view quadrant) {
    ...
    // Find decimal point
    size_t pointPos = val.find('.');
    if (pointPos == std::string_view::npos || pointPos < 2) return 0.0;

    // Split strictly based on NMEA standard (DDDmm.mmmm)
    // The last 2 chars before the dot are ALWAYS minutes.
    size_t splitIdx = pointPos - 2;

    double degrees = 0.0;
    double minutes = 0.0;

    // Parse degrees
    std::from_chars(val.data(), val.data() + splitIdx, degrees);

    // Parse minutes (everything from splitIdx onwards)
    std::from_chars(val.data() + splitIdx, val.data() + val.size(), minutes);

    double result = degrees + (minutes / 60.0);
    if (!quadrant.empty() && (quadrant[0] == 'S' || quadrant[0] == 'W')) {
        result = -result;
    }
    return result;
}
