 ...
 *
 *   Vector3 getBias()
 *     Current gyro bias estimate [rad/s]. Filters that do not expose this
 *     directly (e.g. UKF which uses getState()) must be wrapped in a traits
 *     specialisation — see filter_traits.hpp.
 *
 *   uint32_t getCovarianceFaultCount()
 *     Number of hard covariance resets since construction. Telemetry-visible
 *     diagnostic. Filters without this mechanism should return 0.
 *
 * Compliance
 * ----------
 * Both AttitudeMekf and AttitudeUkf satisfy this concept after the addition
 * of the small adapter shims in filter_traits.hpp. No changes are required
 * to either filter's own header or implementation.
 */
